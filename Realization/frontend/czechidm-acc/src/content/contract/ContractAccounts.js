...
import { ContractAccountManager, AccountManager } from '../../redux';
import AccountTypeEnum from '../../domain/AccountTypeEnum';
import SystemEntityTypeEnum from '../../domain/SystemEntityTypeEnum';

const uiKey = 'contract-accounts-table';
const manager = new ContractAccountManager();
 ...
 * Contract accounts
 *
 * TODO: accounts should be shown (not contractAccount)
 *
 * @author Vít Švanda
 */
class ContractAccountsContent extends Advanced.AbstractTableContent {
  ...
  getContentKey() {
    return 'acc:content.contract.accounts';
  }
  ...
  save(entity, event) {
    ...
    if (Utils.Entity.isNew(formEntity)) {
      const contract = Utils.Entity.getEntity(state, contractManager.getEntityType(), formEntity.contract);
      formEntity.contract = contract.id;
      ...
      <div>
        ...
        <Basic.Panel className="no-border last">
          <Advanced.Table
            ...
            showRowSelection={Managers.SecurityManager.hasAnyAuthority(['CONTRACTACCOUNT_DELETE'])}
            rowClass={({rowIndex, data}) => { return (data[rowIndex]._embedded.account.inProtection) ? 'disabled' : ''; }}
            actions={
              Managers.SecurityManager.hasAnyAuthority(['CONTRACTACCOUNT_DELETE'])
              ?
            ...
            buttons={
              [
                <Basic.Button
                  ...
                  rendered={Managers.SecurityManager.hasAnyAuthority(['CONTRACTACCOUNT_CREATE'])}>
                ...
                ({ rowIndex, data }) => {
                  return (
                    <Advanced.DetailButton
                      ...
                      rendered={Managers.SecurityManager.hasAnyAuthority(['CONTRACTACCOUNT_READ'])}
...
ContractAccountsContent.propTypes = {
  _showLoading: PropTypes.bool,
};
ContractAccountsContent.defaultProps = {
  _showLoading: false,
};
...
export default connect(select)(ContractAccountsContent);
